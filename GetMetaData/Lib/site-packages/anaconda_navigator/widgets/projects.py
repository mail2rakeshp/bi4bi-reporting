...
class ProjectEditor(QWidget):
    ...
    def ordered_widgets(self):
        ...
        ordered_widgets = [self.button_save, self.button_problems, self.button_suggestions, self.editor]
...
class ProjectsWidget(WidgetBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.label_project_location = LabelProjectLocation('<b>Project location</b>')
    ...
    def check_content_change(self):
        ...
        if self.project_path:
            project_config_path = os.path.join(self.project_path, 'anaconda-project.yml')
    ...
    def _upload(self, worker, output, error):
        ...
        if error or errors:
            ...
            self.update_status('Project <b>{0}</b> upload successful'.format(worker.name))
    ...
    def upload(self):
        ...
        if self.editor.is_dirty():
            self.update_status('Saving project <b>{0}</b>'.format(project.name))
            self.editor.save()
        ...
        if not project.problems:
            ...
            msg = 'Uploading project <b>{0}</b> to Anaconda Cloud.'.format(project.name)
            self.update_status(msg)
        else:
            self.update_status('Problems must be fixed before uploading <b>{0}</b>' ''.format(name))
    ...
    def load_project(self, project_path, overwrite=True):
        ...
        project_config_path = os.path.join(project_path, 'anaconda-project.yml')
        data = ''
        if os.path.isfile(project_config_path):
            with open(project_config_path, 'r') as f:
                data = f.read()
