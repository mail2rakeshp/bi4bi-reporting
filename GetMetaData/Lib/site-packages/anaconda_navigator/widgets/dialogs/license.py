...
class LicenseManagerDialog(DialogBase):
    ...
    def __init__(self, parent=None):
        ...
        self.label_info = LabelBase('Manage your Continuum Analytics ' 'license keys.')
        ...
        self.button_contact.clicked.connect(
            lambda v=None: self.sig_url_clicked.emit(self.CONTACT_LINK, 'License Manager')
        )
    ...
    def add_license(self, v=None, path=None):
        ...
        for path in invalid_licenses:
            text = ('File: <b>"{0}"</b>' '<br>is not a valid license file.').format(path)
            self.message_box = MessageBoxInformation(text=text, title="Invalid license file")
    ...
    def remove_license(self, row=None):
        ...
        if row_data:
            text = ('Do you want to remove license for product:<br><br>' '<b>{product}</b> ({issued} - {end_date})')
            text = text.format(
                product=row_data.get('product'), end_date=row_data.get('end_date'), issued=row_data.get('issued')
            )
            self.message_box = MessageBoxRemove(title='Remove license', text=text)
