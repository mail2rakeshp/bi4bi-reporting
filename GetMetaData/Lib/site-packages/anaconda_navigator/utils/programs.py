...
def run_shell_command(cmdstr, **subprocess_kwargs):
    ...
    if 'shell' in subprocess_kwargs and not subprocess_kwargs['shell']:
        raise ProgramError('The "shell" kwarg may be omitted, but if ' 'provided it must be True.')
    else:
...
def run_program(program, args=None, **subprocess_kwargs):
    ...
    if 'shell' in subprocess_kwargs and subprocess_kwargs['shell']:
        raise ProgramError("This function is only for non-shell programs, " "use run_shell_command() instead.")
    ...
    if os.name == 'nt':
        ...
        try:
            ...
            QMessageBox.critical(
                None, _('Run'), _("It was not possible to run this file in "
...
def check_version(actver, version, cmp_op):
    ...
    if (
        is_stable_version(version) and not is_stable_version(actver) and actver.startswith(version)
        and version != actver
    ):
        version = version + 'zz'
    elif (
        is_stable_version(actver) and not is_stable_version(version) and version.startswith(actver)
        and version != actver
    ):
        actver = actver + 'zz'
    ...
    if interpreter:
        if osp.isfile(interpreter) and ('python' in interpreter):
            ...
            with os.fdopen(fd, 'w') as f:
                ...
                f.write(ismod_inst + "\n")
                if version:
                    f.write("print(is_module_installed('%s','%s'))" % (module_name, version))
                else:
...
def is_python_interpreter(filename):
    ...
    if (
        not osp.isfile(real_filename) or encoding.is_text_file(real_filename)
    ...
    try:
        ...
        valid = ("Options and arguments (and corresponding environment " "variables)")
...
def local_test_programs():
    ...
    assert shell_split('-q "d:\\Python de xxxx\\t.txt" -o -a') == ['-q', 'd:\\Python de xxxx\\t.txt', '-o', '-a']
